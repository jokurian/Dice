...
void calcMixedEstimatorLNOProp(Wavefunction& waveLeft, Wavefunction& waveRight, Wavefunction& waveGuide, DQMCWalker& walker, Hamiltonian& ham)
{
  int norbs = ham.norbs;
  int nalpha = ham.nalpha;
  int nbeta = ham.nbeta;
  ...
  size_t nwalk = schd.nwalk;             // number of walkers per process
  double dt = schd.dt;
  int orbitalE = schd.orbitalE;
  ofstream afqmcFile("afqmc.dat", ios::app);

  matPair ref;
  MatrixXcd refSOC;
  std::array<std::complex<double>, 2> hamOverlap;
  std::array<std::complex<double>, 3> hamOverlapOrbital;
  if (schd.soc) {
    MatrixXcd hf = MatrixXcd::Zero(2*norbs, 2*norbs);
    readMat(hf, "ghf.txt");
    refSOC = hf.block(0, 0, 2*norbs, nelec);
  ...
  ArrayXd totalWeights = ArrayXd::Zero(nsweeps);
  ArrayXd totalEnergies = ArrayXd::Zero(nsweeps);
  ArrayXd localorbEnergy = ArrayXd::Zero(nwalk);
  ArrayXd totalorbEnergies = ArrayXd::Zero(nsweeps);
  if (walker.szQ) walker.prepProp(refSOC, ham, dt, ene0.real());
  else walker.prepProp(ref, ham, dt, ene0.real());
  auto calcInitTime = getTime();
  for (int w = 0; w < nwalk; w++) {
    ...
    walkers[w].overlap(waveLeft);  // this initializes the trialOverlap in the walker, used in propagation
    //walkers[w].overlap(waveGuide);  // this initializes the trialOverlap in the walker, used in propagation
    hamOverlapOrbital = walkers[w].orbitalEnergy(waveLeft, ham,orbitalE);

    localorbEnergy(w) = hamOverlapOrbital[2].real(); //(walkers[w].orbitalEnergy(waveLeft,ham,orbitalE)).real();
    localEnergy(w) = (hamOverlapOrbital[0]/hamOverlapOrbital[1]).real();
  }
  
  totalWeights(0) = nwalk * commsize;
  double weightedEnergy = localEnergy.sum();
  MPI_Allreduce(MPI_IN_PLACE, &weightedEnergy, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD); 
  double weightedorbE = localorbEnergy.sum();
  MPI_Allreduce(MPI_IN_PLACE, &weightedorbE, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  totalorbEnergies(0) = weightedorbE/totalWeights(0) ;
  if(orbitalE==-1) totalorbEnergies(0) +=delta.real();
  totalEnergies(0) = weightedEnergy / totalWeights(0) + delta.real(); 
  if (commrank == 0) {	
    double initializationTime = getTime() - calcInitTime;
    afqmcFile << "# block     propTime           eshift          weight             energy          cumulative_energy          walltime         Orbital_Energy\n"; 
	afqmcFile << boost::format(" %5d     %.3e       %.5e     %.5e      %.9e       %9s                %.2e         %.9e \n") % 0 % 0. % totalEnergies(0) % totalWeights(0) % totalEnergies(0) % '-' % initializationTime %(totalorbEnergies(0)); 
    afqmcFile.flush();
    cout << "   Iter        Mean energy          Stochastic error       Walltime         Orbital Energy\n";
    cout << boost::format(" %5d      %.9e      %9s                %.2e        %.9e \n") % 0 % (totalEnergies(0)) % '-' % initializationTime %totalorbEnergies(0); 
  }
  ...
  for (int step = 1; step < nsweeps * nsteps; step++) {
    ...
    if (step % nsteps == 0) { 
      ...
      for (int w = 0; w < walkers.size(); w++) {
        if (weights(w) != 0.) {
          ...
          // energy
          auto hamOverlapOrbital = walkers[w].orbitalEnergy(waveLeft, ham,orbitalE);
          localEnergy(w) = (hamOverlapOrbital[0]/hamOverlapOrbital[1]).real() + delta.real();
          localorbEnergy(w) =  hamOverlapOrbital[2].real();
		  //cout << localEnergy(w); 
		  if (std::isnan(localEnergy(w)) || std::isinf(localEnergy(w))) {
            cout << "local energy:  " << localEnergy(w) << endl;
            cout << "weight:  " << weights(w) << endl;
            cout << "overlap:  " << walkers[w].trialOverlap << "   " << hamOverlapOrbital[1] << endl;
            cout << "ham:  " << hamOverlapOrbital[0] << endl;
            cout << "orthofac:  " << walkers[w].orthoFac << endl;
            exit(0);
            localEnergy(w) = 0.;
            weights(w) = 0.;
    ...
    eneTime += getTime() - init;
    weightedEnergy = (localEnergy * weights).sum();
    weightedorbE = (localorbEnergy * weights).sum();
	MPI_Allreduce(MPI_IN_PLACE, &weightedEnergy, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);	
	MPI_Allreduce(MPI_IN_PLACE, &weightedorbE, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    totalWeights(block) = totalWeight;
    totalorbEnergies(block) = weightedorbE / totalWeight; //+ delta.real();
	totalEnergies(block) = weightedEnergy / totalWeight; 
	if(orbitalE==-1) totalorbEnergies(block) += delta.real();
    ...
    if (commrank == 0){
      if (step * dt < 10. || step < schd.burnIter * nsteps) {
          ...
          averageDenomEql += totalWeights(block);
          averageEnergyEql = averageNumEql / averageDenomEql;
        afqmcFile << boost::format(" %5d     %.3e       %.5e     %.5e      %.9e         %7s                %.2e          %.9e\n") % block % (dt * step) % eshift % totalWeights(block) % totalEnergies(block) % '-' % (getTime() - calcInitTime) %(totalorbEnergies(block)); 
        afqmcFile.flush();
      }
      else {
        averageNum += totalEnergies(block) * totalWeights(block);
        averageDenom += totalWeights(block);
        averageEnergy = averageNum / averageDenom; 
    	afqmcFile << boost::format(" %5d     %.3e       %.5e     %.5e      %.9e         %.9e       %.2e          %.9e\n") % block % (dt * step) % eshift % totalWeights(block) % totalEnergies(block) % averageEnergy % (getTime() - calcInitTime) %(totalorbEnergies(block)); 
        afqmcFile.flush();
      }
      }
      eEstimate = 0.9 * eEstimate + 0.1 * totalEnergies(block);
    ...
    if (commrank == 0) {
      if (step > max(40, schd.burnIter)*nsteps && step % (20*nsteps) == 0) {
        auto ene_err = blocking(totalWeights.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)), totalEnergies.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)));
        auto orbene_err = blocking(totalWeights.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)), totalorbEnergies.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)));
        if (step % (100*nsteps) == 0) {
          int block = step / nsteps;
          if (ene_err[1] > 0.) cout << boost::format(" %5d      %.9e        %.9e        %.2e        %.9e\n") % block % ene_err[0] % ene_err[1] % (getTime() - calcInitTime) %orbene_err[0]; 
          else cout << boost::format(" %5d      %.9e      %9s                %.2e         %.9e\n") % block % ene_err[0] % '-' % (getTime() - calcInitTime) %orbene_err[0]; 
        }
      }
    }
 ...
 // if (commrank == 0) { 
 // 	for (int w = 0; w < walkers.size(); w++) {
 //   		//walkers[w].orthogonalize();
 // 			auto hamOverlapOrbital = walkers[w].orbitalEnergy(waveLeft, ham,10000);
 // 	}
 //   ofstream myfile;
 ...
 // }
  ...
  if (commrank == 0) {
    ...
    ofstream samplesFile(fname);
    for (int i = 0; i < nsweeps; i++) {
        samplesFile << boost::format("%.7e      %.10e \n") % totalWeights(i) % totalorbEnergies(i);
    }
    samplesFile.close();
    if (measureCounter > 40) {
		if(orbitalE==-2) blocking(totalWeights.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)), totalEnergies.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)), "blocking.tmp");
		else{
			 blocking(totalWeights.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)), totalorbEnergies.head(measureCounter).tail(measureCounter - max(40, schd.burnIter)), "blocking.tmp");
    		 ofstream blockingfile;
		     blockingfile.open("blocking.tmp", std::ios_base::app); 	
			 blockingfile <<  "Initial Orbital Energy: "<<setprecision(12) << totalorbEnergies(0)<<"\n";
			 blockingfile << "Cholesky error: " <<setprecision(6) << delta.real() <<"\n";
			 blockingfile.close();
		}
	}
  }
