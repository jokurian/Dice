...
GHFMultislater::GHFMultislater(Hamiltonian& ham, std::string fname, int pnact, int pncore, bool prightQ) 
{
...
void GHFMultislater::getSample(Eigen::MatrixXcd& sampleDet)
{
  return;
};
...
void GHFMultislater::forceBias(Eigen::MatrixXcd& psi, Hamiltonian& ham, Eigen::VectorXcd& fb)
{
  int norbs = ham.norbs;
  int nelec = refDet.size();
  int nchol = ham.nchol;
...
void GHFMultislater::oneRDM(Eigen::MatrixXcd& psi, Eigen::MatrixXcd& rdmSample) 
{ 
  int norbs = psi.rows();
  int nelec = psi.cols();
  size_t ndets = ciCoeffs.size();
...
std::array<std::complex<double>, 3> GHFMultislater::orbitalEnergy(std::array<Eigen::MatrixXcd, 2>& psi, Hamiltonian& ham,int orbital)
{
return std::array<std::complex<double>, 3>();
};
std::array<std::complex<double>, 3> GHFMultislater::orbitalEnergy(Eigen::MatrixXcd& psi, Hamiltonian& ham,int orbital)
{   
 return std::array<std::complex<double>, 3> ();
};
